...
class PacketSniffer:
    ...
    def start_raw_socket(self):
        ...
        self.address = (
            config["packetsniffer"]["listenaddress"],
        ...
        listenerthread = threading.Thread(
            target=self.accept_raw_connection, args=()
        )  # threads the accept
            ...
            for sessionID, connection in sessions_list.items():
                ...
                if connection.details == r_address[0]:
                    ...
                    sharkthread = threading.Thread(
                        target=self.listener, args=(str(i))
                    )  # initalises the listener thread
            ...
            if config["packetsniffer"]["debugPrint"]:
                while True:
                    try:
                        ...
                        ethernet_header_tuple = struct.unpack("!6s6sH", ethernet_header)

                        source_mac = ":".join(
                            "{:02x}".format(x) for x in ethernet_header_tuple[0]
                        )  # grabs the source mac

                        destination_mac = ":".join(
                            "{:02x}".format(x) for x in ethernet_header_tuple[1]
                        )  # grabs the dest mac
                        ...
                        if ethertype == 0x0806:  # arp header
                            ...
                            if opcode == 1:
                                print(
                                    f"Source Mac: {source_mac} SourceIP: "
                        ...
                        if transport_protocol == 6:  # tcp
                            ...
                            packetbyte += 20
                            print(
                                f"Source Mac: {source_mac} Destination Mac ",
                                destination_mac,
