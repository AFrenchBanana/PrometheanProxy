...
Session::Session(const std::string &serverAddress, int port)
    : serverAddress(serverAddress), port(port), sock(INVALID_SOCKET), ctx(nullptr), ssl(nullptr) {}

bool Session::initWinsock() {
    WSADATA wsaData;
    ...
    logger.log("Winsock initialized");
    return true;
}
...
bool Session::connectToServer() {
    ...
    // Cast SOCKET to int for SSL_set_fd on Windows.
...
bool Session::sendData(const std::string &data) {
    ...
    for (size_t i = 0; i < total_length; i += chunk_size) {
        ...
        while (sent < chunk.size()) {
            int n = SSL_write(ssl, chunk.data() + sent, chunk.size() - sent);
            if (n <= 0) {
...
std::string Session::authentication(const std::string &authKey) {
    ...
    if (getsockname(sock, reinterpret_cast<sockaddr*>(&local_addr), &addr_len) == SOCKET_ERROR) {
...
bool Session::connectSocket() {
    ...
    if (connect(sock, reinterpret_cast<sockaddr*>(&server_addr), sizeof(server_addr)) != 0) {
        logger.error("Connection failed with error: " + std::to_string(WSAGetLastError()));
        closesocket(sock);
        logger.warn("Socket closed");
        return false;
    }
