...
int calculateSleepTime(int timer, int jitter) {
    logger.log("Calculating Sleep Time with timer = " + std::to_string(timer) + " and jitter = " + std::to_string(jitter));
    int sign = (rand() % 2 == 0) ? 1 : -1;
    int sleepTime = timer + sign * (rand() % (jitter + 1));
    logger.log("Raw sleep time calculated: " + std::to_string(sleepTime));
    if (sleepTime < 0) {
        logger.log("Sleep time negative; reverting to timer value: " + std::to_string(timer));
        return timer;
    }
    return sleepTime;
}
...
size_t WriteCallback(void* contents, size_t size, size_t nmemb, std::string* s) {
    ...
    try {
        ...
        logger.error("Memory allocation error in WriteCallback: " + std::string(e.what()));
        return 0;
    }
    return newLength;
}
...
std::tuple<int, std::string, std::string> getRequest(const std::string& url) {
    logger.log("Performing GET request to: " + url);
    CURL* curl;
    CURLcode res;
    ...
    if (curl) {
        ...
        logger.log("GET request succeeded. Response code: " + std::to_string(response_code));
        logger.log("Response body received: " + response_body);
        curl_easy_cleanup(curl);
    } else {
        logger.error("Failed to initialize curl in getRequest for URL: " + url);
    }
    curl_global_cleanup();
...
std::tuple<int, std::string> postRequest(const std::string& url, const std::string& jsonData) {
    ...
    if (curl) {
        ...
        res = curl_easy_perform(curl);
        if (res != CURLE_OK) {
            logger.error("POST request failed: " + std::string(curl_easy_strerror(res)));
            curl_easy_cleanup(curl);
        ...
        curl_easy_getinfo(curl, CURLINFO_RESPONSE_CODE, &response_code);
        logger.log("POST request succeeded. Response code: " + std::to_string(response_code));
        curl_easy_cleanup(curl);
        curl_slist_free_all(headers);
    } else {
        logger.error("Failed to initialize curl in postRequest for URL: " + url);
    }
    curl_global_cleanup();
...
std::tuple<int, std::string, int> httpConnection(const std::string& address) {
    ...
    if (response_code == -1) {
        logger.error("httpConnection POST request failed, response code -1");
        return std::make_tuple(-1, "", -1);
    }

    try {
        if (response_code == 200) {
            logger.log("httpConnection succeeded. Parsing response...");
            Json::Value data;
            Json::CharReaderBuilder readerBuilder;
            std::string errs;
            std::istringstream s(response_body);
            if (!Json::parseFromStream(readerBuilder, s, &data, &errs)) {
                logger.error("JSON parsing failed in httpConnection: " + errs);
            ...
            if (data.isMember("timer") && data.isMember("uuid") && data.isMember("jitter")) {
                ...
                logger.log("Parsed connection parameters: timer = " + std::to_string(timer) +
                           ", uuid = " + ID + ", jitter = " + std::to_string(jitter));
                return std::make_tuple(timer, ID, jitter);
            } else {
                logger.error("Invalid JSON response in httpConnection: " + response_body);
                throw std::runtime_error("Invalid JSON response: " + response_body);
            }
        } else {
            logger.error("Server responded with error in httpConnection: " + std::to_string(response_code) + " " + response_body);
            throw std::runtime_error("Failed to connect to server: " + std::to_string(response_code) + " " + response_body);
        }
    } catch(const std::exception& e) {
        logger.error("Exception in httpConnection: " + std::string(e.what()));
...
std::tuple<int, std::string, int> httpReconnect(const std::string& address, const std::string& user_id, int jitter, int timer) {
    logger.log("Starting httpReconnect for user_id: " + user_id);
    std::string hostname = getHostname();
    std::vector<std::string> ipAddresses = getIPAddresses();
    ...
    if (response_code == -1) {
        logger.error("httpReconnect POST request failed");
        return std::make_tuple(-1, "", -1);
    }
    ...
    try {
        ...
        logger.error("Exception in httpReconnect: " + std::string(e.what()));
        std::cerr << "Exception: " << e.what() << std::endl;
        throw;
    }
}
        ...
        if (response_code == -1) {
            ...
            try {
                if (response_code == 200) {
                    ...
                    #elif _WIN32
                        logger.log("Creating Windows thread to handle response");
